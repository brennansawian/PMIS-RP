$(document).ready(function () {
    ...
    // function to initialize a DataTable on a given selector.
    function initializeDataTable(tableSelector, columnDefs) {
        if ($(tableSelector).length && !$.fn.DataTable.isDataTable(tableSelector)) {
            try {
                $(tableSelector).DataTable({
                    ...
                    columnDefs: columnDefs || [] // Use provided column definitions
                });
                console.log("DataTables initialized for: " + tableSelector);
            } catch (e) {
                console.error("Error initializing DataTables for " + tableSelector + ":", e);
            }
        }
    }

    $('#modalmylist-modal').on('shown.bs.modal', function () {
            // For 'myProgramsAcceptedTable':
            // 0: Program ID, 1: Name, 2: Description, 3: Phases, 4: Details
            // We want 'Details' (4) to be highest priority, and 'Description' (2) to be lowest.
            initializeDataTable('#myProgramsAcceptedTable', [
                { responsivePriority: 1, targets: 4 },   // 'Details' button column
                { responsivePriority: 2, targets: 0 },   // Program ID
                { responsivePriority: 3, targets: 1 },   // Program Name
                { responsivePriority: 10001, targets: 2 } // Program Description (will hide first)
            ]);

            // For 'myProgramsRejectedTable', it has fewer columns, but we can still define
            initializeDataTable('#myProgramsRejectedTable', [
    ...
    // listeners modals to initialize tables only when they are shown.
    $('#modalmylist-modal').on('shown.bs.modal', function () {
        initializeDataTable('#myProgramsAcceptedTable');
        initializeDataTable('#myProgramsRejectedTable');
    });
    $('#modalongoinglist-modal').on('shown.bs.modal', function () {
        initializeDataTable('#ongoingTable');
    });
    $('#modalupcominglist-modal').on('shown.bs.modal', function () {
        initializeDataTable('#upcomingTable');
    });
    $('#modalclosedlist-modal').on('shown.bs.modal', function () {
        initializeDataTable('#closedTable');
    });

    $('#modalarchivedlist-modal').on('shown.bs.modal', function () {
        initializeDataTable('#completedTable');
    });

    $('#modalcompletedlist-modal').on('shown.bs.modal', function () {
        initializeDataTable('#archivedTable');
    });
    ...
    $("#two-item-row").on("click", function () { $(".b-customize").removeClass("col-lg-3 col-lg-12").addClass("col-lg-6", 300); });
    $("#three-item-row").on("click", function () { $(".b-customize").removeClass("col-lg-6 col-lg-12").addClass("col-lg-3", 300); });


    $('button[data-program-code]:not([data-phaseid])').on("click", function(event) {
